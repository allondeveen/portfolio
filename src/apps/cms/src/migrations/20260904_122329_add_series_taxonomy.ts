...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`articles\` ADD \`_articles_articles_order\` text;`);
  await db.run(sql`ALTER TABLE \`articles\` ADD \`series_id\` text REFERENCES series(id);`);
  await db.run(
    sql`CREATE INDEX \`articles__articles_articles_order_idx\` ON \`articles\` (\`_articles_articles_order\`);`,
  );
  await db.run(sql`CREATE INDEX \`articles_series_idx\` ON \`articles\` (\`series_id\`);`);
  await db.run(sql`ALTER TABLE \`_articles_v\` ADD \`version__articles_articles_order\` text;`);
  await db.run(
    sql`ALTER TABLE \`_articles_v\` ADD \`version_series_id\` text REFERENCES series(id);`,
  );
  await db.run(
    sql`CREATE INDEX \`_articles_v_version_version__articles_articles_order_idx\` ON \`_articles_v\` (\`version__articles_articles_order\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`_articles_v_version_version_series_idx\` ON \`_articles_v\` (\`version_series_id\`);`,
  );
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`series_id\` text REFERENCES series(id);`,
  );
  await db.run(
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_articles\`("id", "slug", "title", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status") SELECT "id", "slug", "title", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status" FROM \`articles\`;`,
  );
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new__articles_v\`("id", "parent_id", "version_slug", "version_title", "version_meta_title", "version_meta_description", "version_meta_image_id", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest") SELECT "id", "parent_id", "version_slug", "version_title", "version_meta_title", "version_meta_description", "version_meta_image_id", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest" FROM \`_articles_v\`;`,
  );
