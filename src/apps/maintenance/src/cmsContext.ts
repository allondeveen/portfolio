...
type CMSClient = TRPCClient<MaintenanceContentRouter>;
...
export function createCMSClient(env: Env, signal: AbortSignal): CMSClient {
  ...
  function getToken() {
    tokenPromise ??= requestAccessToken({
      tokenEndpoint: new URL("/oauth/token/", `https://${env.CMS_HOST}`).toString(),
  ...
  return createTRPCClient<MaintenanceContentRouter>({
    links: [
      retryLink<MaintenanceContentRouter>({
      ...
      httpLink({
        url: new URL("/trpc", `https://${env.CMS_HOST}`),
        async headers({ op }) {
